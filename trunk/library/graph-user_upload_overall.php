<?php
/*******************************************************************
* Extension name: graph-user_upload_overall.php                    *
*                                                                  *
* Description:    this graph extension procduces a query of the    *
* overall uploads made by a particular user on a daily, monthly,   *
* and yearly basis.                                                *
*                                                                  *
...
function daily($username) {
	...
	include 'library/opendb.php';
        ...
        $sql = "SELECT UserName, SUM(AcctInputOctets) As Upload, DAY(AcctStartTime) AS Day from radacct where username='$username' group by Day;";
	...
	$count = 0;	
		...
		// +----------+--------+------+
        	...
        	$user = $ent[0];	// username
        	$uploads = ($ent[1]/1024/1024);	// total uploads on that specific day
        ...
        echo "<br/> <center> <h4>Upload statistics for user $user</h4> <br/> </center> ";

        //graph information to create
        $graph = new BAR_GRAPH("hBar");
        $graph->labels = $array_days;
        $graph->values = $array_uploads;
        ...
        echo "
                        <thead>
                                <tr>
                                <th colspan='10'>Upload statistics</th>
                                </tr>
                        </thead>
                ";
        echo "<thread> <tr>
	...
	echo "<br/> Total uploads of <u>$total_uploads</u> for user: <u>$user</u> <br/>";
...
function monthly($username) {
	...
	include 'library/opendb.php';
        ...
        $sql = "SELECT UserName, SUM(AcctInputOctets) As Upload, MONTHNAME(AcctStartTime) AS Month from radacct where username='$username' group by Month;";
        $res = mysql_query($sql) or die('Query failed: ' . mysql_error());
	...
	$count = 0;			
		...
		// The table that is being procuded is in the format of:
		// +----------+--------+--------+
		// | UserName | Upload | Month  |
		// +----------+--------+--------+

        	$user = $ent[0];	// username
        	$uploads = ($ent[1]/1024/1024);	// total uploads on that specific month
...
function yearly($username) {
	...
	include 'library/opendb.php';
        ...
        $sql = "SELECT UserName, SUM(AcctInputOctets) As Upload, YEAR(AcctStartTime) AS Year from radacct where username='$username' group by Year;";
        $res = mysql_query($sql) or die('Query failed: ' . mysql_error());
