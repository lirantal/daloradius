...
$l['maintenance'] = [
    ...
    'help' => '<h2 class="fs-6">Перед началом</h2><p>Открытая запись аккаунтинга не обязательно является устаревшей. Этот инструмент не проверяет активность пользователя или подключение к NAS.</p><h2 class="fs-6">Действия</h2><ul><li><strong>Закрыть сессии</strong> сохраняет зарегистрированное использование и помечает выбранные строки как закрытые с помощью <code>Admin-Reset</code>. Пользователи при этом не отключаются от NAS.</li><li><strong>Удалить записи</strong> безвозвратно удаляет выбранные открытые строки и данные об их использовании.</li></ul><h2 class="fs-6">Область и подтверждение</h2><p>Сопоставление имени пользователя выполняется точно в соответствии с сортировкой базы данных. Дата соответствует сессиям, начавшимся строго до полуночи этой даты в часовом поясе базы данных/сервера аккаунтинга.</p><p>Перед подтверждением просмотрите каждую операцию. Предварительный просмотр содержит не более 100 строк, истекает через 10 минут и может быть использован один раз. Изменённые или более не подходящие строки пропускаются.</p>',
    'close' => 'Закрыть сессии',
    'delete' => 'Удалить записи',
    'details' => 'Подробности',
    'openWarning' => 'Открытые сессии могут быть активны.',
    'openHelp' => 'Только записи с отсутствующим или устаревшим нулевым временем остановки имеют право на обработку. Определение неактивности не выполняется.',
    'dateHelp' => 'Область даты означает, что НАЧАЛО сессии должно быть ДО выбранной даты, строго до 00:00:00 в часовом поясе базы данных/сервера аккаунтинга (а не в часовом поясе браузера). Это не ограничение по времени последней активности.',
    'selectTitle' => 'Выбор сессий',
    'closeSummary' => 'Сохранить данные аккаунтинга и пометить выбранные сессии как закрытые.',
    'deleteSummary' => 'Безвозвратно удалить выбранные открытые записи аккаунтинга.',
    'closeHelp' => 'Записи закрываются сейчас с причиной ручного завершения Admin-Reset. Пользователи при этом не отключаются от NAS. Зарегистрированная продолжительность и счётчики трафика сохраняются без изменений; будущие обновления аккаунтинга не гарантируются.',
    'deleteHelp' => 'Удаление необратимо. Зарегистрированное использование удаляется безвозвратно, поэтому отчёты или итоговые показатели использования могут измениться. Закрытая история этим инструментом не удаляется.',
    'scope' => 'Выбор области',
    'username' => 'Имя пользователя',
    'date' => 'Начало сессии до даты',
    'value' => 'Имя пользователя или дата (YYYY-MM-DD)',
    'filterHelp' => 'Изменение этого фильтра требует нового предварительного просмотра.',
    'filterDetails' => 'Сопоставление имени пользователя выполняется с учётом сортировки базы данных аккаунтинга; поиск по шаблону не поддерживается.',
    'preview' => 'Предварительный просмотр совпадающих записей',
    'previewTitle' => 'Предварительный просмотр',
    'previewDetails' => 'Подробности предварительного просмотра',
    'count' => '%d совпадающих записей на момент подсчёта; %d записей в этом предварительном просмотре. Максимум %d за операцию, сортировка по ID аккаунтинга. Изменять можно только отображаемые записи. Для следующей партии повторите предварительный просмотр; количество может измениться во время обновлений аккаунтинга.',
    'concurrency' => 'Подтверждение истекает через 10 минут и может быть использовано один раз. Записи, изменённые обновлениями Interim/Stop или более не соответствующие условиям, пропускаются. Новые совпадающие записи никогда не добавляются в эту операцию.',
    'id' => 'ID аккаунтинга',
    'nas' => 'Адрес NAS',
    'start' => 'Начало сессии',
    'activity' => 'Время зарегистрированного обновления',
    'seconds' => 'Зарегистрированные секунды',
    'input' => 'Зарегистрированные входящие байты',
    'output' => 'Зарегистрированные исходящие байты',
    'activityHelp' => 'Время обновления отображается только если схема аккаунтинга предоставляет acctupdatetime. Отсутствующие значения отображаются как тире. Эти значения базы данных не доказывают отсутствие активности или подключение к NAS.',
    'closeSelectionOne' => '%d сессия выбрана',
    'closeSelectionMany' => '%d сессий выбрано',
    'deleteSelectionOne' => '%d запись выбрана',
    'deleteSelectionMany' => '%d записей выбрано',
    'closeConfirmOne' => 'Закрыть %d сессию',
    'closeConfirmMany' => 'Закрыть %d сессий',
    'deleteConfirmOne' => 'Безвозвратно удалить %d запись',
    'deleteConfirmMany' => 'Безвозвратно удалить %d записей',
    'empty' => 'Нет совпадающих открытых записей. Ничего не изменено.',
    'result' => '%s: фактически затронуто %d, пропущено %d изменённых или более не подходящих записей, ошибок: %d. Перед любой дальнейшей операцией повторите предварительный просмотр.',
    'invalid' => 'Недопустимый запрос, фильтр, CSRF-токен или истёкшее подтверждение. Ничего не изменено. Создайте новый предварительный просмотр.',
    'error' => 'Не удалось прочитать записи аккаунтинга. Операция не выполнена.',
];
