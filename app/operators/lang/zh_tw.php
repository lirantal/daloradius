...
$l['maintenance'] = [
    'title' => '未關閉工作階段維護',
    'history' => '刪除帳務歷史記錄（所有記錄）',
    'help' => '<h2 class="fs-6">開始前</h2><p>尚未關閉的帳務記錄不一定已過期。此工具不會檢查使用者活動或 NAS 連線。</p><h2 class="fs-6">操作</h2><ul><li><strong>關閉工作階段</strong>會保留已記錄的使用量，並以 <code>Admin-Reset</code> 將選取的資料列標示為已關閉。此操作不會將使用者與 NAS 中斷連線。</li><li><strong>刪除記錄</strong>會永久移除選取的尚未關閉資料列及其使用量資料。</li></ul><h2 class="fs-6">範圍與確認</h2><p>使用者名稱比對會依據資料庫定序精確進行。日期會以帳務資料庫／伺服器時區，比對在該日期午夜之前嚴格開始的工作階段。</p><p>確認前請先預覽每個操作。每個預覽最多包含 100 筆資料列，10 分鐘後過期，且只能使用一次。已變更或不再符合資格的資料列會被略過。</p>',
    'close' => '關閉工作階段',
    'delete' => '刪除記錄',
    'details' => '詳細資訊',
    'openWarning' => '未關閉工作階段可能仍在使用中。',
    'openHelp' => '只有停止時間遺失或為舊式零值的記錄符合資格。不會執行閒置偵測。',
    'dateHelp' => '日期範圍表示工作階段開始時間必須早於所選日期，也就是在帳務資料庫／伺服器時區中嚴格早於 00:00:00（不是瀏覽器時區）。這不是最後活動時間的截止條件。',
    'selectTitle' => '選取工作階段',
    'closeSummary' => '保留帳務資料並將選取的工作階段標示為已關閉。',
    'deleteSummary' => '永久移除選取的尚未關閉帳務記錄。',
    'closeHelp' => '記錄現在會以手動終止原因 Admin-Reset 關閉。這不會將使用者與 NAS 中斷連線。已記錄的持續時間和流量計數器會原樣保留；不保證未來的帳務更新。',
    'deleteHelp' => '刪除不可復原。已記錄的使用量會永久移除，報告或使用量總計可能會變更。此工具不會刪除已關閉的歷史記錄。',
    'scope' => '選取範圍',
    'username' => '使用者名稱',
    'date' => '工作階段開始早於日期',
    'value' => '使用者名稱或日期（YYYY-MM-DD）',
    'filterHelp' => '變更此篩選條件需要新的預覽。',
    'filterDetails' => '使用者名稱比對遵循帳務資料庫定序；不支援萬用字元搜尋。',
    'preview' => '預覽相符記錄',
    'previewTitle' => '預覽',
    'previewDetails' => '預覽詳細資訊',
    'count' => '%d 個相符記錄（計數時）；此預覽中有 %d 筆記錄。每個操作最多 %d 筆，依帳務 ID 排序。只有顯示的記錄可以變更。若要處理另一批次，請再次預覽；帳務更新期間計數可能會變更。',
    'concurrency' => '確認會在 10 分鐘後過期且只能使用一次。被 Interim/Stop 更新變更或不再符合資格的記錄會被略過。新符合條件的記錄絕不會加入此操作。',
    'id' => '帳務 ID',
    'nas' => 'NAS 位址',
    'start' => '工作階段開始時間',
    'activity' => '已記錄的更新時間',
    ...
    'input' => '已記錄輸入位元組',
    'output' => '已記錄輸出位元組',
    'activityHelp' => '更新時間僅在帳務結構描述提供 acctupdatetime 時顯示。缺少的值會顯示為破折號。這些資料庫值不代表閒置狀態或 NAS 連線。',
    'closeSelectionOne' => '%d 個工作階段已選取',
    'closeSelectionMany' => '%d 個工作階段已選取',
    'deleteSelectionOne' => '%d 筆記錄已選取',
    'deleteSelectionMany' => '%d 筆記錄已選取',
    'closeConfirmOne' => '關閉 %d 個工作階段',
    'closeConfirmMany' => '關閉 %d 個工作階段',
    'deleteConfirmOne' => '永久刪除 %d 筆記錄',
    'deleteConfirmMany' => '永久刪除 %d 筆記錄',
    'empty' => '沒有相符的尚未關閉記錄。未進行任何變更。',
    'result' => '%s：實際影響 %d 筆，略過 %d 筆已變更或不符合資格的記錄，%d 筆失敗。進行任何後續操作前，請再次預覽。',
    'invalid' => '無效的請求、篩選條件、CSRF 權杖或已過期的確認。未進行任何變更。請建立新的預覽。',
    'error' => '無法讀取帳務記錄。未執行任何操作。',
];
