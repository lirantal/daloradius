...
$l['maintenance'] = [
    'title' => 'オープンセッションのメンテナンス',
    'history' => 'アカウンティング履歴を削除（全レコード）',
    'help' => '<h2 class="fs-6">開始する前に</h2><p>オープン中のアカウンティングレコードが、必ずしも古いレコードとは限りません。このツールは、ユーザーのアクティビティやNASへの接続状態を確認しません。</p><h2 class="fs-6">操作</h2><ul><li><strong>セッションを終了</strong>すると、記録済みの使用量を保持したまま、選択した行を<code>Admin-Reset</code>で終了済みとしてマークします。NAS上のユーザーは切断されません。</li><li><strong>レコードを削除</strong>すると、選択したオープン中の行とその使用量データを完全に削除します。</li></ul><h2 class="fs-6">範囲と確認</h2><p>ユーザー名の照合は、データベースの照合順序に従って完全一致で行われます。日付は、アカウンティングデータベース／サーバーのタイムゾーンで、その日の午前0時より前に開始されたセッションに一致します。</p><p>確認する前に、すべての操作をプレビューしてください。プレビューには最大100行が含まれ、有効期限は10分で、1回だけ使用できます。変更済みまたは対象外になった行はスキップされます。</p>',
    'close' => 'セッションを終了',
    'delete' => 'レコードを削除',
    'details' => '詳細',
    'openWarning' => 'オープン中のセッションは、まだアクティブな可能性があります。',
    'openHelp' => '対象となるのは、停止時刻が欠落しているか、従来形式のゼロであるレコードだけです。非アクティブ状態の検出は行いません。',
    'dateHelp' => '日付の範囲は、選択した日付より前にセッションが開始されたものです。アカウンティングデータベース／サーバーのタイムゾーンにおける00:00:00を基準とし、境界を含みません（ブラウザーのタイムゾーンではありません）。最終アクティビティの締切ではありません。',
    'selectTitle' => 'セッションを選択',
    'closeSummary' => 'アカウンティングデータを保持し、選択したセッションを終了済みとしてマークします。',
    'deleteSummary' => '選択したオープン中のアカウンティングレコードを完全に削除します。',
    'closeHelp' => 'レコードは手動終了原因Admin-Resetで直ちに終了済みになります。NAS上のユーザーは切断されません。記録済みの継続時間とトラフィックカウンターは変更せず保持されます。今後のアカウンティング更新は保証されません。',
    'deleteHelp' => '削除は元に戻せません。記録済みの使用量は完全に削除され、レポートや使用量の合計が変わる可能性があります。このツールでは終了済みの履歴は削除されません。',
    'scope' => '範囲を選択',
    'username' => 'ユーザー名',
    'date' => '日付より前に開始されたセッション',
    'value' => 'ユーザー名または日付（YYYY-MM-DD）',
    'filterHelp' => 'このフィルターを変更するには、新しいプレビューが必要です。',
    'filterDetails' => 'ユーザー名の照合はアカウンティングデータベースの照合順序に従います。ワイルドカード検索はサポートされていません。',
    'preview' => '一致するレコードをプレビュー',
    'previewTitle' => 'プレビュー',
    'previewDetails' => 'プレビューの詳細',
    'count' => 'カウント時点で%d件の一致するレコード、今回のプレビューに%d件のレコード。1操作あたり最大%d件で、アカウンティングID順です。表示されたレコードだけを変更できます。別のバッチを取得するにはプレビューを繰り返してください。アカウンティング更新中に件数が変わる可能性があります。',
    'concurrency' => '確認の有効期限は10分で、1回だけ使用できます。Interim／Stop更新によって変更されたレコード、または対象外になったレコードはスキップされます。新たに一致したレコードがこの操作に追加されることはありません。',
    'id' => 'アカウンティングID',
    'nas' => 'NASアドレス',
    'start' => 'セッション開始時刻',
    'activity' => '記録された更新時刻',
    ...
    'input' => '記録された入力バイト数',
    'output' => '記録された出力バイト数',
    'activityHelp' => '更新時刻は、アカウンティングスキーマがacctupdatetimeを提供している場合にのみ表示されます。値がない場合はダッシュで表示されます。これらのデータベース値は、非アクティブ状態やNASへの接続を証明するものではありません。',
    'closeSelectionOne' => '%d件のセッションを選択しました',
    'closeSelectionMany' => '%d件のセッションを選択しました',
    'deleteSelectionOne' => '%d件のレコードを選択しました',
    'deleteSelectionMany' => '%d件のレコードを選択しました',
    'closeConfirmOne' => '%d件のセッションを終了',
    'closeConfirmMany' => '%d件のセッションを終了',
    'deleteConfirmOne' => '%d件のレコードを完全に削除',
    'deleteConfirmMany' => '%d件のレコードを完全に削除',
    'empty' => '一致するオープン中のレコードがありません。何も変更されませんでした。',
    'result' => '%s：実際に影響を受けた件数%d、変更済みまたは対象外としてスキップされたレコード%d件、失敗%d件。次の操作を行う前に、もう一度プレビューしてください。',
    'invalid' => 'リクエスト、フィルター、CSRFトークン、または期限切れの確認が無効です。何も変更されませんでした。新しいプレビューを作成してください。',
    'error' => 'アカウンティングレコードを読み取れません。操作は実行されませんでした。',
];
