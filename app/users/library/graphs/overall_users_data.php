...
if (!empty($username)) {
    if ($type === 'yearly') $sql = "SELECT YEAR(AcctStartTime), %s FROM %s WHERE username='%s' AND AcctStopTime>0 GROUP BY YEAR(AcctStartTime) ORDER BY YEAR(AcctStartTime) DESC LIMIT 36";
    elseif ($type === 'monthly') $sql = "SELECT CONCAT(LEFT(MONTHNAME(AcctStartTime), 3), ' (', YEAR(AcctStartTime), ')'), %s FROM %s WHERE username='%s' AND AcctStopTime>0 GROUP BY YEAR(AcctStartTime), MONTH(AcctStartTime) ORDER BY YEAR(AcctStartTime) DESC, MONTH(AcctStartTime) DESC LIMIT 36";
    else $sql = "SELECT DATE(AcctStartTime), %s FROM %s WHERE username='%s' AND AcctStopTime>0 GROUP BY DATE(AcctStartTime) ORDER BY DATE(AcctStartTime) DESC LIMIT 36";
...
$title = $category === 'login' ? sprintf('login statistics for user %s', $username) : sprintf('traffic in %s by user %s', $category, $username);
